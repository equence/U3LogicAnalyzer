...
static struct sr_key_info sr_key_info_config[] = {
	...
	{SR_CONF_TEST_MODE, SR_T_STRING, "test_mode",
		"Test mode", NULL},
	{SR_CONF_HOT_PLUG, SR_T_UINT64, "hot_plug",
		"hot plug", NULL},
	{SR_CONF_TRIGGER_OFFSET, SR_T_UINT64, "trigger_offset",
		"trigger offset", NULL},
	{SR_CONF_THRESHOLD_VALUE_1, SR_T_UINT64, "threshold_value_1",
		"threshold value_1", NULL},
	{SR_CONF_THRESHOLD_VALUE_2, SR_T_UINT64, "threshold_value_2",
		"threshold value_2", NULL},
	{SR_CONF_REPEAT_ACQUISITION, SR_T_BOOL, "repeat_acquisition",
		"repeat acquisition", NULL},
	{SR_CONF_HARDWARE_VERSION, SR_T_UINT64, "hardware_version",
		"hardware version", NULL},
	{SR_CONF_USB_VERSION, SR_T_UINT64, "usb_version",
		"usb version", NULL},
	{SR_CONF_SEGMENT_CLEAR, SR_T_BOOL, "segment_clear",
		"segment clear", NULL},
	ALL_ZERO
};
		...
		uint32_t key, GVariant **data)
{
	...
	//DbgPrint("Enter file:%s, API: %s, line:%d", __FILE__, __func__, __LINE__);
	...
	if (ret == SR_ERR_CHANNEL_GROUP) {
		sr_err("%s: No channel group specified.",
			(sdi) ? sdi->driver->name : "unknown");
		//DbgPrint("%s: No channel group specified.",
			//(sdi) ? sdi->driver->name : "unknown");
	}
